...
<script>
export default {
  props: ["tickerName", "modelValue"],

  emits: ["update:modelValue", "closeGraph"],
  ...
  methods: {
    ...
    getGraphContainerWidth() {
      if (!this.$refs.graphContainer) return 0;

      return this.$refs.graphContainer.offsetWidth;
    },

    getGraphItemWidth() {
      if (!this.$refs.graphItem) return 0;
